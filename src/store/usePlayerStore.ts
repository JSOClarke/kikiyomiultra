...
export const usePlayerStore = create<PlayerState>()(
  ...
  setActiveBook: (book) => set((state) => {
    // Re-hydrate the dynamically split subtitle array instantly if the network payload dictates it
        ...
        for (let j = 0; j < parts.length; j++) {
           ...
           if (/[、,]/.test(parts[j]) || j === parts.length - 1) {
               if (current.trim().length > 1) {
                   finalSubs.push({ ...sub, id: `${sub.id}_c${j}`, text: current.trim() });
               }
               current = "";
           }
        }
    ...
    if (newSplitSetting) {
      activeBook.originalSubtitles.forEach(sub => {
        ...
        for (let j = 0; j < parts.length; j++) {
           ...
           if (/[、,]/.test(parts[j]) || j === parts.length - 1) {
               if (current.trim().length > 1) {
                   newSubs.push({ ...sub, id: `${sub.id}_c${j}`, text: current.trim() });
               }
               current = "";
           }
        }
