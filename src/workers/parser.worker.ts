...
function segmentText(allText: string, breakByCommas: boolean = false): { subs: Subtitle[], duration: number } {
    const subs: Subtitle[] = [];
    // The worker doesn't have DOM access. 
    // Usually Intl.Segmenter is available in modern workers.
    let time = 0;
    try {
        const segmenter = new (Intl as any).Segmenter('ja', { granularity: 'sentence' });
        ...
        for (const { segment } of segments) {
            ...
            if (breakByCommas) {
                ...
                for (let i = 0; i < subParts.length; i++) {
                    current += subParts[i];
                    if (/[、,]/.test(subParts[i]) || i === subParts.length - 1) {
                        if (current.trim().length > 1) {
                            subs.push({ id: `sub_${time}`, startTime: time, endTime: time + 1, text: current.trim(), isRead: false });
                            time += 1;
                        }
                        current = "";
                    }
                ...
                subs.push({ id: `sub_${time}`, startTime: time, endTime: time + 1, text: text, isRead: false });
                time += 1;
            }
        }
    } catch (e) {
        // Ultimate fallback if Intl.Segmenter is totally missing in some worker env
        const splitRegex = breakByCommas ? /([。！？!?、,])/ : /([。！？!?])/;
        const testRegex = breakByCommas ? /[。！？!?、,]/ : /[。！？!?]/;
        const parts = allText.split(splitRegex);
        let current = "";
        for (let i = 0; i < parts.length; i++) {
            current += parts[i];
            if (testRegex.test(parts[i]) || i === parts.length - 1) {
                if (current.trim().length > 1) {
                    subs.push({ id: `sub_${time}`, startTime: time, endTime: time + 1, text: current.trim(), isRead: false });
                    time += 1;
                }
