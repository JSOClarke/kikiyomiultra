...
export const PomodoroTimer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isPomodoroActive && pomodoroTimeLeft > 0) {
      interval = setInterval(() => {
        setPomodoroState({ pomodoroTimeLeft: pomodoroTimeLeft - 1 });
      }, 1000);
  ...
  const toggleTimer = () => setPomodoroState({ isPomodoroActive: !isPomodoroActive });
  ...
  if (isSidebarCollapsed) {
    return (
      ...
      >
        <div className="relative w-10 h-10 flex items-center justify-center bg-surface border border-black/5 dark:border-white/5 rounded-full shadow-sm">
            ...
            <div className={`text-[0.6rem] font-bold ${pomodoroMode === 'focus' ? 'text-primary' : 'text-green-500'} font-mono`}>
